...
import li.yansan.clean.dispatcher.handler.UnsupportedHandlerException;

public class DefaultDispatcherUseCase<UPayload, UBody> implements DispatcherUseCase<UPayload, UBody> {
	...
	public UseCaseResponse<UBody> execute(UseCaseRequest<UPayload> request) {
		Validate.notNull(request, "Dispatcher cannot execute a null request. Check the caller of %s",
